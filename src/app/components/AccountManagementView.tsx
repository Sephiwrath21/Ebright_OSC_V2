...
function roleAccent(roleType: string): { bg: string; text: string; dot: string } {
  const palette: Record<string, { bg: string; text: string; dot: string }> = {
    ...
    department: { bg: "bg-sky-100", text: "text-sky-700", dot: "bg-sky-500" },
    branch: { bg: "bg-teal-100", text: "text-teal-700", dot: "bg-teal-500" },
    "regional manager": {
      bg: "bg-cyan-100",
      text: "text-cyan-700",
      dot: "bg-cyan-500",
    },
    od: { bg: "bg-violet-100", text: "text-violet-700", dot: "bg-violet-500" },
    hod: { bg: "bg-fuchsia-100", text: "text-fuchsia-700", dot: "bg-fuchsia-500" },
    staff: { bg: "bg-slate-100", text: "text-slate-700", dot: "bg-slate-500" },
  };
  return (
...
function positionAccent(position: string | null): {
  ...
  const palette: Record<string, { bg: string; text: string; dot: string }> = {
    BM: { bg: "bg-orange-100", text: "text-orange-700", dot: "bg-orange-500" },
    "FT EXEC": { bg: "bg-blue-100", text: "text-blue-700", dot: "bg-blue-500" },
    "FT COACH": { bg: "bg-lime-100", text: "text-lime-700", dot: "bg-lime-600" },
    "PT COACH": {
      bg: "bg-green-100",
      text: "text-green-700",
      dot: "bg-green-500",
    },
    "FT HOD": { bg: "bg-pink-100", text: "text-pink-700", dot: "bg-pink-500" },
    "FT CEO": {
      bg: "bg-purple-100",
      text: "text-purple-700",
      dot: "bg-purple-500",
    },
    INTERN: { bg: "bg-pink-100", text: "text-pink-700", dot: "bg-pink-500" },
    ADMIN: { bg: "bg-indigo-100", text: "text-indigo-700", dot: "bg-indigo-500" },
  };
  return (
    palette[p] ?? {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between gap-4 px-6 py-5 border-b border-slate-200 bg-gradient-to-b from-white to-slate-50">
          <div className="flex items-center gap-3 min-w-0">
            <div className="rounded-full w-11 h-11 bg-indigo-100 text-indigo-700 font-semibold flex items-center justify-center shrink-0">
              {initials(user.full_name, user.email)}
            </div>
            <div className="min-w-0">
              <h3
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50/60 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="inline-flex items-center h-9 px-4 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition-all duration-200"
          >
            Done
          </button>
        </div>
    ...
    >
      <div className="flex items-center gap-2">
        <select
          value={value}
          onChange={(e) => setValue(e.target.value)}
          className="h-10 flex-1 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-800 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:bg-slate-50 disabled:text-slate-400 transition-all duration-200"
        >
    ...
    <SectionShell icon={<Shield className="w-4 h-4" />} title="Role">
      <div className="flex items-center gap-2">
        <select
          ...
          className="h-10 flex-1 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-800 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all duration-200"
        >
          {roles.map((r) => (
            <option key={r.role_id} value={r.role_id}>
              {prettyRole(r.role_type)}
    ...
    >
      <div className="flex items-center gap-2">
        <input
          ...
          onChange={(e) => setValue(e.target.value)}
          autoComplete="off"
          className="h-10 flex-1 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all duration-200"
        />
        <SaveButton disabled={!dirty} pending={pending} onClick={save} />
      </div>
      <div className="mt-2">
