    ...
    <div className="min-h-full bg-slate-50 dark:bg-slate-950">
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-10">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-6">
          ...
          <ChevronRight className="w-4 h-4 text-slate-400" />
          <span className="text-slate-900 dark:text-slate-100 font-medium">My Tickets</span>
        </nav>
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">All support tickets across branches.</p>
          </div>
          <Link
            href="/crm/ticket/new"
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="flex items-center gap-4 px-5 pt-4 pb-3 border-b border-slate-100 dark:border-slate-800">
            <div className="relative flex-1 max-w-xs">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-950 dark:border-slate-500 dark:text-slate-100"
              />
            </div>
          </div>

          {/* Status tabs */}
          <div className="flex gap-1 px-5 py-2 border-b border-slate-100 dark:border-slate-800 overflow-x-auto">
            {STATUS_TABS.map(tab => (
              <button
                key={tab}
              ...
              >
                {tab}
                <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${activeTab === tab ? "bg-blue-500 text-white" : "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400"}`}>
                  {countByStatus(tab)}
                </span>
              </button>
            ))}
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {loading ? (
                  <tr><td colSpan={8} className="px-5 py-12 text-center">
                    <div className="mx-auto mb-2 h-6 w-6 animate-spin rounded-full border-2 border-slate-200 dark:border-slate-700 border-t-blue-600" />
                    <p className="text-sm text-slate-400">Loading tickets…</p>
                  </td></tr>
                ) : error ? (
                  <tr><td colSpan={8} className="px-5 py-12 text-center text-sm text-red-500 dark:text-red-400">Couldn&apos;t load tickets: {error}</td></tr>
                ) : tickets.length === 0 ? (
                  ...
                  <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <td className="px-5 py-3 text-[11px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{fmtDate(t.createdAt)}</td>
                    <td className="px-5 py-3 text-[11px] text-slate-400 whitespace-nowrap">{ageOf(t.createdAt)}</td>
                    <td className="px-5 py-3">
                      <button className="rounded-lg p-1.5 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors" title="View">
