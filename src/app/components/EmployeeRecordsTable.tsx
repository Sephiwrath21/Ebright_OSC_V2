...
interface Props {
  ...
  /** Pre-fills and disables the Branch advanced filter (2026-08-26, see
   *  conversation) — for a scoped viewer embedding this table in the /home
   *  "Employee Records" modal, so it's visibly locked to their own branch
   *  rather than offering a picker into data `rows` doesn't even contain.
   *  Full-access viewers get both omitted, identical to /employee-folder. */
  lockedBranch?: string;
  /** Same as lockedBranch, for Department. Mutually exclusive with it, same
   *  as the underlying data always is. */
  lockedDepartment?: string;
  /** Omits the "Employee Records" heading (2026-08-26, see conversation) —
   *  for the Employee Overview modal, whose own dialog title already says
   *  "Employee Records"; a second copy right below it read as a duplicate. */
  hideHeading?: boolean;
}

// Extracted verbatim from EmployeeOverviewView.tsx (2026-08-26, see
// conversation) — the search/filter/table portion, unchanged in behavior for
// /employee-folder, now also reusable standalone inside the Employee Records
// modal via lockedBranch/lockedDepartment/hideHeading.
export default function EmployeeRecordsTable({
  rows,
  overdueTaskCounts,
  lockedBranch,
  lockedDepartment,
  ...
  const isLocked = Boolean(lockedBranch || lockedDepartment);
  ...
  // the draft here and in resetFilters/applyFilters below. Locked values
  // (2026-08-26) seed both draft and applied state so the table starts
  // already filtered, and resetFilters below returns to the lock instead of
  // clearing it.
  const [branchFilterDraft, setBranchFilterDraft] = useState(lockedBranch ?? "");
  const [departmentFilterDraft, setDepartmentFilterDraft] = useState(lockedDepartment ?? "");
  ...
  const [departmentFilter, setDepartmentFilter] = useState(lockedDepartment ?? "");
  ...
  function resetFilters() {
    ...
    setYearDraft("");
    setMonthDraft("");
    setBranchFilterDraft(lockedBranch ?? "");
    setDepartmentFilterDraft(lockedDepartment ?? "");
    setStatusFilter("");
    setSearch("");
    setYear("");
    setMonth("");
    setBranchFilter(lockedBranch ?? "");
    setDepartmentFilter(lockedDepartment ?? "");
    setPage(1);
  }

  return (
    <section aria-labelledby={hideHeading ? undefined : "employees-list-heading"}>
      {!hideHeading && (
      ...
      >
        ...
        {/* Advanced filters — collapsible, same pattern as the Exit
            ...
            have one or the other set. Locked (2026-08-26): both selects
            stay disabled regardless of the other's draft value, so a
            scoped viewer can see but not change their own lock. */}
        {advancedOpen && (
          <div className="w-full flex flex-wrap items-center gap-2 sm:gap-4">
            <select
              value={branchFilterDraft}
              disabled={isLocked || Boolean(departmentFilterDraft)}
              onChange={(e) => {
            ...
            <select
              ...
              disabled={isLocked || Boolean(branchFilterDraft)}
              onChange={(e) => {
