...
export default function FormsPage() {
  ...
  // A non-admin with exactly ONE branch is locked to it. A multi-branch user
  // may pick freely among THEIR branches — regardless of which branch is
  // selected in the topbar — so they can file a lead for any branch they manage
  // without first switching the topbar view (the old bug: locking to the viewed
  // branch meant their other branches "couldn't submit").
  const singleBranchName = !isAdmin && accessibleNames.length === 1 ? accessibleNames[0] : null
  const lockedBranchName = singleBranchName
  // Admins see every branch (from the live branch directory, minus HR which has
  // no lead pipeline) so a newly-added branch appears automatically; everyone
  // else sees only the branches they're explicitly linked to.
  const adminBranchNames = ctxBranches
    .map((b) => b.name)
    .filter((n) => !/ebright\s*hr/i.test(n))
    .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }))
  const branchOptions = isAdmin ? adminBranchNames : accessibleNames

  useEffect(() => {
    // Single-branch non-admin → force their one branch. Multi-branch users
    // default to the topbar-selected branch when it's one they can submit for,
    // but the field stays changeable so they can pick any of their branches.
    if (singleBranchName) { setBranch(singleBranchName); return }
    if (!isAdmin && selectedBranch && accessibleNames.includes(selectedBranch.name)) {
      setBranch((b) => b || selectedBranch.name)
    }
  }, [singleBranchName, isAdmin, selectedBranch?.name, accessibleNames.join('|')])
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok) {
        // The API returns JSON { error }; showing res.text() dumped the raw
        // `{"error":"…"}` into the toast. Parse it so the branch sees a plain
        // sentence (e.g. why the CNS form was rejected).
            ...
            <Fade>
              <Group label="Preferred branch near you">
                ...
                {lockedBranchName ? (
                  ...
                  <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>
                    Choose which of your branches this lead belongs to — you can file for any branch you manage.
                  </p>
                ) : null}
              </Group>
