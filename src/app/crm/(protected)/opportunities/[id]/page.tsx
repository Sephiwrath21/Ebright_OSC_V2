...
import { ArrowLeft, MapPin, User, Tag, Clock, ArrowRight, ArrowRightLeft, Plus, GraduationCap, Users, PenLine, MessageCircle } from 'lucide-react'
...
import { ContactEditCard } from '@/components/crm/opportunities/contact-edit-card'
...
import { QaqcPanel } from '@/components/crm/opportunities/qaqc-panel'
import { LeadWhatsAppPanel } from '@/components/crm/opportunities/lead-whatsapp-panel'
import { getQaqcForOpportunity, getQaqcCaps } from '@/server/actions/qaqc'
...
export default async function OpportunityDetailPage({ params }: PageProps) {
  ...
  // QAQC verification — panel renders only for Super Admin / Regional Manager /
  // Operation (the panel itself hides when the viewer has no access + unverified).
  const [qaqcState, qaqcCaps] = await Promise.all([
    getQaqcForOpportunity(opp.id),
    getQaqcCaps(),
  ])
  ...
  // Free-text remark the parent typed on the registration/Wix form. Shown even
  // when empty so BMs can tell "no remark" apart from "not checked".
  const formRemarks = (cExt.remarks ?? '').trim()
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-6">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[320px_1fr]">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="space-y-2 text-sm">
              ...
              {/* Email + Parent's Contact are inline-editable; changes are
                  logged as a note ("changed from X to Y"). */}
              <ContactEditCard contactId={contact.id} email={contact.email} phone={contact.phone} />
              <Row
        ...
        {/* Right column — QAQC + Remarks + Notes + Activity timeline */}
        <div className="space-y-4">
          {/* QAQC verification — Super Admin / Regional Manager / Operation. */}
          <QaqcPanel opportunityId={opp.id} initial={qaqcState} caps={qaqcCaps} />

          {/* Parent's free-text remark from the registration/Wix form. Always
              rendered — an empty-state message shows when the parent left it
              blank, so "no remark" reads differently from "section missing". */}
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            ...
            {formRemarks ? (
              ...
              <p className="rounded-lg border border-dashed border-slate-200 bg-slate-50 px-3 py-2 text-sm italic text-slate-400 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-500">
                No remarks provided on the form.
              </p>
            )}
          </section>

          {/* WhatsApp conversation with this parent, on the branch's own
              connected number. Same thread as the kanban popup and the
              Conversations inbox — they all read crm_message. */}
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <h3 className="mb-3 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
