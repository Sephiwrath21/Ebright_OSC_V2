...
// Render CRM + ticket pages at a fixed desktop width on every device and let
// the browser scale it to fit — i.e. phones get the "zoomed-out desktop" view
// automatically, without the user having to toggle "Request desktop site".
// 980px matches Chrome/Brave's built-in Desktop-site width, so the layout
// looks identical to flipping that switch. No `initialScale` on purpose:
// omitting it lets the browser compute the fit-to-screen scale (the zoom-out);
// setting it to 1 would instead render 980px at 1:1 and force horizontal
// scrolling. Users can still pinch-zoom. This viewport export overrides Next's
// default `width=device-width` for the whole /crm/(protected) subtree, which
// includes the ticket module (app/crm/(protected)/tickets/**).
...
export default async function CrmProtectedLayout({
  ...
  // Scoped-department identity for the ticket-module topbar lock. The
  // agency-view CEO is intentionally excluded (it keeps the broad view), so a
  // department is "locked" only when it is a genuine sub_type-scoped account.
  const scopedDept = session?.user?.email ? scopedDepartmentForEmail(session.user.email) : null
  const lockedDept = scopedDept && !isAgencyViewAccount(session!.user.email) ? scopedDept : null
  ...
  const sessionProp = session
    ? {
        user: {
          ...
          tktRole,
          tktBranchIds,
          departmentSubType: lockedDept?.subType ?? null,
          departmentName: lockedDept?.name ?? null,
        },
      }
    : {
        user: {
