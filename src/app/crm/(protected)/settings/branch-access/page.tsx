...
import { Plus, Trash2, Loader2, Search, Building2, ArrowLeft, Landmark } from 'lucide-react'
...
import { isDepartmentBranch, departmentForBranchName } from '@/lib/crm/departments'
  ...
  const filteredAvailable = useMemo(() => {
    ...
    return available.filter(
      (b) =>
        ...
        // Let the department NAME find its branch ("marketing" → Ebright Marketing).
        (departmentForBranchName(b.name)?.name ?? '').toLowerCase().includes(q),
    )
  }, [available, search])

  // Departments are grantable too, not just branches — they're ordinary
  // crm_branch rows, but they're hidden from the topbar's account list, so
  // showing them mixed in here made them look like centres. Split them into
  // their own group and label them by department name.
  const departmentRows = useMemo(
    () => filteredAvailable.filter((b) => isDepartmentBranch(b.name)),
    [filteredAvailable],
  )
      ...
      <DialogContent className="max-w-lg">
                  ...
                  {filteredAvailable.length === 0 ? (
                    ...
                    <>
                      {departmentRows.length > 0 && (
                        <>
                          <div className="bg-slate-50 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:bg-slate-900/60 dark:text-slate-500">
                            Departments
                          </div>
                          <ul className="divide-y divide-slate-100 dark:divide-slate-700">
                            {departmentRows.map((b) => {
                              const isSelected = selected.has(b.id)
                              const dept = departmentForBranchName(b.name)
                              return (
                                <li
                                  key={b.id}
                                  className={cn(
                                ...
                                >
                                  ...
                                  <span className="ml-auto text-[11px] text-slate-400">{b.name}</span>
                                </li>
                              )
                            })}
                          </ul>
        ...
        <DialogFooter>
          ...
          >
            ...
            Grant access to {selected.size || 0} {selected.size === 1 ? 'scope' : 'scopes'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
