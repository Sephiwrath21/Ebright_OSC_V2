...
import { departmentForSubType, DEPARTMENT_TKT_BRANCH_NUMBERS } from '@/lib/crm/departments'
import { z } from 'zod'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const isOthersPlatform = platformSlug === 'other' || platformSlug === 'others'

    // Platform-scope enforcement (server-authoritative — the form hides the
    // other platform cards for these accounts, but a tampered POST could still
    // send platformSlug: 'aone'). Only a BRANCH account — one that files
    // against a real centre — may raise platform tickets (Aone / GHL / Lead /
    // …). Every other account (department accounts like finance@/hr@, HQ, or a
    // user with no centre branch) may only raise internal "Others → department"
    // tickets. Real super_admins are unrestricted.
    if (!isOthersPlatform && ctx.role !== 'super_admin') {
      const isBranchAccount =
        !ctx.departmentSubType &&
        ctx.branchIds.length > 0 &&
        (
          await prisma.tkt_branch.findMany({
            where: { tenant_id: ctx.tenantId, id: { in: ctx.branchIds } },
            select: { branch_number: true },
          })
        ).some((b) => !DEPARTMENT_TKT_BRANCH_NUMBERS.has(b.branch_number))
      if (!isBranchAccount) {
        return err('This account can only submit internal (Others → department) tickets', 403)
      }
    }

    // Branch-scope enforcement. The TicketForm dropdown already restricts
    // visible branches via GET /api/crm/tkt-branches, but a tampered submission
    // could send any UUID — so re-check here.
    //   • scoped DEPARTMENT account → only its OWN department branch (it is a
    //     forced platform_admin, so the role check below would wave it through).
    //   • regular user → only a branch it's linked to.
    //   • real super_admin / platform_admin → any branch in the tenant.
    if (ctx.departmentSubType) {
      const dept = departmentForSubType(ctx.departmentSubType)
      const deptBranch = dept
        ? await prisma.tkt_branch.findFirst({
            where: { tenant_id: ctx.tenantId, branch_number: dept.tktBranchNumber },
            select: { id: true },
          })
        : null
      if (!deptBranch || deptBranch.id !== branchId) {
        return err('Department accounts can only submit under their own department', 403)
      }
    } else if (ctx.role === 'user' && !ctx.branchIds.includes(branchId)) {
