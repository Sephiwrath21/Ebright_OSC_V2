...
export async function PATCH(
  ...
  try {
    ...
    // Scope check — mirror the status route. A scoped department account may
    // only assign tickets directed to its department (by sub_type); a real
    // platform_admin only tickets on its assigned platforms. Without this a
    // finance account could re-assign an HR (or any other) ticket.
