...
export async function GET(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    // Role-scoped access. Mirror the list route (route.ts GET) exactly, so a
    // ticket that shows up in a user's list can also be opened:
    //   • scoped DEPARTMENT account → only tickets of its sub_type (it is a
    //     forced platform_admin with NO platform links, so the platform check
    //     below would wrongly 403 every one of its own tickets).
    //   • regular user → any ticket from one of their branch(es); or, if they
    //     have no branch link, only their own submissions. (Previously this
    //     allowed ONLY own submissions, so a branch user got "Access denied"
    //     opening a co-worker's ticket that the list had shown them.)
    //   • real platform_admin → only its assigned platforms.
    if (ctx.departmentSubType) {
      if (ticket.sub_type !== ctx.departmentSubType) return err('Access denied', 403)
    } else if (ctx.role === 'user') {
      const allowed =
        ctx.branchIds.length > 0
          ? ctx.branchIds.includes(ticket.branch_id)
          : ticket.user_id === ctx.userId
      if (!allowed) return err('Access denied', 403)
