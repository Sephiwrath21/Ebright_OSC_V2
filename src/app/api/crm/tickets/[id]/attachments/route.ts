  ...
  try {
    ...
    const enriched = await Promise.all(
      attachments.map(async (a) => ({
        ...
        // Drive files are private — serve via the auth-gated proxy route.
        // S3 keys get a short-lived presigned URL.
        downloadUrl:   a.url.startsWith('gdrive:')
          ? `/api/crm/tickets/${id}/attachments/${a.id}`
          : await getPresignedDownloadUrl(a.url).catch(() => null),
      })),
    )
