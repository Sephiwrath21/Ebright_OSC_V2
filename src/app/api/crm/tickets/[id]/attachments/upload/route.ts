...
import { isTicketDriveConfigured, ticketDriveFolderId, uploadPrivateFile } from '@/lib/googleDrive'
...
export async function POST(
  ...
  try {
    ...
    // Prefer Google Drive (ticket-media folder) when configured; fall back to
    // S3. Drive file ids are stored as `gdrive:<id>` in the same `url` column
    // so the serve/list/delete routes can tell the two backends apart without
    // a schema change. Drive uploads stay PRIVATE — viewing is proxied through
    // the auth-gated [attId] GET route (no public "anyone with link").
    let storedUrl: string
    try {
      if (isTicketDriveConfigured()) {
        const folderId = ticketDriveFolderId()!
        // Prefix with the ticket id + timestamp so the Drive folder stays
        // browsable and two same-named screenshots never collide.
        const driveName = `${id}__${Date.now()}__${file.name}`
        const { fileId } = await uploadPrivateFile(buffer, driveName, mimeType, folderId)
        storedUrl = `gdrive:${fileId}`
      } else {
