...
export async function GET(req: NextRequest) {
  try {
    ...
    const roleRows = session?.user?.id
      ? await prisma.crm_user_branch.findMany({ where: { userId: session.user.id }, select: { role: true, branchId: true } })
      : []
    const isRegionalManager = roleRows.some((r) => r.role === 'REGIONAL_MANAGER')
    const canSeeBranch = isElevatedUser || isRegionalManager
    // The RM's OWN managed branches (their REGIONAL_MANAGER links) — distinct
    // from access.branchIds, which also holds a BRANCH_MANAGER link to every
    // other branch. Used to scope the "main" headline drill-in so it matches
    // the headline card (their managed subset), not all 23 branches.
    const rmOwnBranchIds = roleRows.filter((r) => r.role === 'REGIONAL_MANAGER').map((r) => r.branchId)

    const regionNames = (r: Scope): string[] =>
      r === 'A' ? REGIONS.A : r === 'B' ? REGIONS.B : r === 'C' ? REGIONS.C : []
    const regionOf = (name: string): 'A' | 'B' | 'C' | null =>
      REGIONS.A.includes(name) ? 'A' : REGIONS.B.includes(name) ? 'B' : REGIONS.C.includes(name) ? 'C' : null

    // A regional manager's OWN region = where most of their managed branches
    // sit (mirrors the dashboard's viewerRegion). They may drill only this
    // region's card; the other two are read-only counts to them, so this guards
    // the PII of regions they don't manage.
    let rmOwnRegion: 'A' | 'B' | 'C' | null = null
    if (isRegionalManager && rmOwnBranchIds.length > 0) {
      const owned = await prisma.crm_branch.findMany({ where: { tenantId, id: { in: rmOwnBranchIds } }, select: { name: true } })
      const counts: Record<'A' | 'B' | 'C', number> = { A: 0, B: 0, C: 0 }
      for (const b of owned) { const rg = regionOf(b.name); if (rg) counts[rg] += 1 }
      const top = (['A', 'B', 'C'] as const).reduce((best, r) => (counts[r] > counts[best] ? r : best), 'A')
      rmOwnRegion = counts[top] > 0 ? top : null
    }

    // Region scope is only meaningful for those who see region cards. Elevated
    // admins may drill ANY region; a regional manager may drill ONLY their own
    // region (rmOwnRegion) — other regions' cards are read-only counts to them.
    // A plain branch manager gets no region scope at all. This prevents reaching
    // a whole region's lead PII by crafting scope=A.
    const canUseRegionScope =
      isElevatedUser || (isRegionalManager && scope === rmOwnRegion)

    // Resolve which branches this scope covers.
    let branchWhere: { tenantId: string; name?: { in: string[] }; id?: { in: string[] } }
    if ((scope === 'A' || scope === 'B' || scope === 'C') && canUseRegionScope) {
      // A region card was clicked. The card shows the WHOLE region's total
      // (elevated and RM alike), so the drill-in must too — otherwise the list
      // (previously all-branches for a non-elevated RM) didn't match the count.
      branchWhere = { tenantId, name: { in: regionNames(scope) } }
    } else if (elevated) {
      // Headline drill-in, all-branches admin view.
      branchWhere = { tenantId, name: { in: Object.keys(BRANCH_CODES).filter((n) => !ELEVATED_DASHBOARD_EXCLUDE.has(n)) } }
    } else {
      // Headline drill-in for everyone else. For an RM viewing all branches the
      // headline now spans EVERY branch they can access, so the drill matches
      // (allowedBranchIds = all their branches); a specific-branch pick narrows
      // it; a plain BM gets their own branch(es).
