...
export async function GET(req: NextRequest) {
  try {
    ...
    // Regional managers get the three region cards too (all three, own
    // highlighted). When region cards are shown, the branch counting runs over
    // ALL regioned branches; the caller's own "main"/trend stay scoped via
    // `scopeBranchIds` below. Suppressed once they've narrowed to one branch.
    const isRM = access?.role === 'REGIONAL_MANAGER'
    const showRegions = !viewAsBranch && (isElevatedUser || isRM)

    // Elevated users can pin the dashboard to ONE region via ?region=A|B|C —
    // this is the "view as the regional manager" jump from the topbar. The
    // headline + trend then scope to that region and it becomes the highlighted
    // card, exactly as an RM of that region sees it. Ignored for non-elevated
    // users (they're already scoped to what they manage) and when a single
    // branch is selected.
    const rawRegion = req.nextUrl.searchParams.get('region')
    const regionScope: 'A' | 'B' | 'C' | null =
      isElevatedUser && !viewAsBranch && (rawRegion === 'A' || rawRegion === 'B' || rawRegion === 'C')
        ? rawRegion
        : null
    ...
    // Region + code now come straight from crm_branch (populated by the branch
    // admin UI), so a newly-added branch appears in the region cards + branch
    // list automatically — no hardcoded BRANCH_CODES / REGIONS to maintain.
    // When region cards are shown (elevated OR regional manager), count over
    // every regioned branch (A/B/C); otherwise scope to the caller's branches.
    const branchWhere = showRegions
      ? { tenantId, region: { in: ['A', 'B', 'C'] } }
    ...
    // A regional manager is ALSO linked as BRANCH_MANAGER to every branch
    // OUTSIDE their region (so they can view all kanbans), so
    // `access.branchIds` — and therefore `allowedBranchIds` — spans all three
    // regions. Their OWN region is defined strictly by their
    // REGIONAL_MANAGER-role links; use those for the headline/trend scope +
    // the highlighted region. Without this the "main" block sums every region
    // (91 instead of the region's own total) and `viewerRegion` ties to A.
    let rmOwnBranchIds: string[] | null = null
    if (isRM && session?.user?.id) {
      const rmLinks = await prisma.crm_user_branch.findMany({
        where: { tenantId, userId: session.user.id, role: 'REGIONAL_MANAGER' },
        select: { branchId: true },
      })
      rmOwnBranchIds = rmLinks.map((l) => l.branchId)
    }

    // The headline "main" scope:
    //   - Region-pinned elevated view (?region=X) → that region's branches.
    //   - A specific branch selected (viewAsBranch) → just that branch.
    //   - Elevated OR a regional manager viewing all their branches → EVERY
    //     counted branch. "View all branches" means the grand total; a regional
    //     manager can access all branches (BM links to each), so their headline
    //     spans all of them, NOT only their managed region. The region cards
    //     still break it down per region, and `viewerRegion` (their managed
    //     region) stays highlighted — computed separately below so widening the
    //     headline doesn't move the highlight.
    //   - Plain branch manager → their own branch(es).
    const scopeBranchIds = regionScope
      ? branches.filter((b) => b.region === regionScope).map((b) => b.id)
      : viewAsBranch
        ? [viewAsBranch]
        : (elevated || isRM)
          ? branches.map((b) => b.id)
    ...
    for (const b of branches) {
      branchMetrics.set(b.id, {
        ...
        branchName: b.name,
        code: b.code ?? '',
        region: b.region === 'A' || b.region === 'B' || b.region === 'C' ? b.region : null,
        ...zero(),
      })
    }
    ...
    // Aggregate by region letter (from crm_branch.region on each metric).
    function aggregate(region: 'A' | 'B' | 'C'): BranchMetrics {
      const list = Array.from(branchMetrics.values()).filter((m) => m.region === region)
    ...
    function sumMetrics(list: BranchMetrics[]): BranchMetrics {
      return {
        ...
        BUF: list.reduce((s, x) => s + x.BUF, 0),
        conversionRate: 0, confirmedRate: 0, showUpRate: 0, enrolmentRate: 0,
      }
    }

    // The caller's OWN branches (a regional manager's region subset, or the
    // whole counted set for an elevated all-branches view).
    const scopeMetrics = scopeBranchIds
      .map((id) => branchMetrics.get(id))
      .filter((x): x is BranchMetrics => !!x)

    // Headline "Main" block:
    //   - Region-pinned elevated view (?region=X): that region's total.
    //   - Elevated OR regional-manager all-branches view: sum of A+B+C — the
    //     grand total across every branch (Marketing etc. excluded).
    //   - Otherwise (specific branch, or a plain BM): the caller's scoped set.
    const main: BranchMetrics = regionScope
      ? sumMetrics([{ A: regionA, B: regionB, C: regionC }[regionScope]])
      : (elevated || (isRM && !viewAsBranch))
        ? sumMetrics([regionA, regionB, regionC])
        : sumMetrics(scopeMetrics)
    Object.assign(main, computeRates(main))

    // Which region card is highlighted:
    //   - Region-pinned elevated view → the pinned region.
    //   - Regional manager → the region most of their managed branches sit in.
    //   - Everyone else → none.
    let viewerRegion: 'A' | 'B' | 'C' | null = regionScope
    if (!viewerRegion && isRM) {
      // The RM's own region = where most of their MANAGED branches
      // (rmOwnBranchIds) sit — NOT the headline scope, which now spans every
      // branch. Computed from the managed set so widening the headline to all
      // branches doesn't drift the highlight to whichever region has the most
      // branches overall.
      const counts: Record<'A' | 'B' | 'C', number> = { A: 0, B: 0, C: 0 }
      for (const id of rmOwnBranchIds ?? []) {
        const rg = branchMetrics.get(id)?.region
        if (rg) counts[rg] += 1
      }
      const top = (['A', 'B', 'C'] as const).reduce((best, r) => (counts[r] > counts[best] ? r : best), 'A')
      viewerRegion = counts[top] > 0 ? top : null
    }
    ...
    // Region → branch codes, sourced from the metrics (dynamic; a new branch's
    // code shows up automatically). Used by the region-card subtitles.
    const regionCodes = (r: 'A' | 'B' | 'C') =>
      Array.from(branchMetrics.values())
        .filter((m) => m.region === r)
        .sort((a, b) => a.branchName.localeCompare(b.branchName, undefined, { numeric: true }))
        .map((m) => m.code || m.branchName)
    // The caller's own branches, ordered — for the "your branches" labels +
    // the regional manager / multi-branch trial-schedule picker.
    const scopeOrdered = [...scopeMetrics].sort((a, b) =>
      a.branchName.localeCompare(b.branchName, undefined, { numeric: true }),
    )
    ...
    return NextResponse.json({
      ...
      // Per-branch bar chart + table stay elevated-only (regional managers get
      // region-level cards, not other regions' per-branch detail).
      branches: elevated ? orderedBranches : [],
      regionMap: showRegions
        ? { A: regionCodes('A'), B: regionCodes('B'), C: regionCodes('C') }
        : { A: [], B: [], C: [] },
      elevated,
      // True when the three region cards should render (elevated OR regional
      // manager). `viewerRegion` is the RM's own region for highlighting.
      showRegions,
      viewerRegion,
      // When set, the dashboard is pinned to one region (elevated ?region=X):
      // the client scopes the headline drill-in to that region and labels the
      // main block accordingly.
      regionScope,
      ...
      scopedBranchName: elevated
        ? null
        : scopeOrdered.length === 1
          ? scopeOrdered[0].branchName
          : `All ${scopeOrdered.length} branches`,
      ...
      scopedBranches:
        elevated || scopeOrdered.length <= 1
          ? null
          : scopeOrdered.map((b) => ({ branchId: b.branchId, branchName: b.branchName })),
