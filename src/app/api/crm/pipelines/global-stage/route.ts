...
import { createStageAllPipelines, deleteStageAllPipelines, updateStageAllPipelines } from '@/server/actions/pipelines'
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { shortCode, ...fields } = (await req.json()) as {
      shortCode: string
      name?: string
      color?: string
    ...
    const result = await updateStageAllPipelines(ctx.tenantId, ctx.userId, shortCode, fields)
    return NextResponse.json(result)
  } catch (err) {
    console.error('[PATCH /api/crm/pipelines/global-stage]', err)
    const message = err instanceof Error ? err.message : 'Internal server error'
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
