...
export async function GET() {
  try {
    ...
    // `hidden` is a self-provisioned column (not in the Prisma model) — attach
    // it to each stage so the settings UI can render the eye toggle.
    const hiddenIds = await getHiddenStageIds(ctx.tenantId)
    const withHidden = pipelines.map((p) => ({
      ...p,
      stages: p.stages.map((s) => ({ ...s, hidden: hiddenIds.has(s.id) })),
    }))

    return NextResponse.json({ pipelines: withHidden, canManageGlobal: ctx.canManageGlobal })
  } catch (err) {
