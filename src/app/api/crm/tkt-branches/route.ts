...
import { departmentForSubType } from '@/lib/crm/departments'
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Branch scope:
    //   - scoped DEPARTMENT account (finance, hr, …): ONLY its own department
    //     branch. It is force-promoted to platform_admin to clear role gates,
    //     but must never see or file under other branches — so it is scoped
    //     here explicitly, ahead of the platform_admin "see all" rule below.
    //   - super_admin / real platform_admin: every branch in the tenant.
    //   - regular user: only the branches they're explicitly linked to via
    //     tkt_user_branch (so the ticket form's auto-default lands on the
    //     user's own branch, not branch 01).
    ...
    if (ctx.departmentSubType) {
      ...
      if (!dept) return Response.json([])
      where.branch_number = dept.tktBranchNumber
