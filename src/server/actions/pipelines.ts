...
const StageUpdateSchema = z.object({
  ...
  /** Visibility toggle (eye icon). Stored via raw SQL — see the note below. */
  hidden: z.boolean().optional(),
})

// `crm_stage.hidden` is self-provisioned (like the QAQC columns) rather than
// declared in the Prisma model, so it can be added on a live DB without a
// migration and without breaking every existing stage query the moment the
// model changes. All reads/writes of `hidden` go through raw SQL, guarded by
// this once-per-process ensure.
let stageHiddenEnsured = false
export async function ensureStageHiddenColumn(): Promise<void> {
  if (stageHiddenEnsured) return
  await prisma.$executeRawUnsafe(
    `ALTER TABLE crm.crm_stage ADD COLUMN IF NOT EXISTS hidden boolean NOT NULL DEFAULT false`,
  )
  stageHiddenEnsured = true
}

/** Ids of hidden stages for a tenant (optionally within one pipeline). */
export async function getHiddenStageIds(tenantId: string, pipelineId?: string): Promise<Set<string>> {
  await ensureStageHiddenColumn()
  const rows = pipelineId
    ? await prisma.$queryRawUnsafe<{ id: string }[]>(
        `SELECT id FROM crm.crm_stage WHERE "tenantId" = $1 AND "pipelineId" = $2 AND hidden = true`,
        tenantId, pipelineId,
      )
    : await prisma.$queryRawUnsafe<{ id: string }[]>(
        `SELECT id FROM crm.crm_stage WHERE "tenantId" = $1 AND hidden = true`,
        tenantId,
      )
...
export async function updateStage(
  ...
  // `hidden` isn't a Prisma-model field (self-provisioned) — pull it out and
  // apply it via raw SQL; everything else goes through the normal update.
  const { hidden, ...prismaFields } = parsed

  if (Object.keys(prismaFields).length > 0) {
    await prisma.crm_stage.update({
      where: { id: stageId },
      data: { ...prismaFields, updatedAt: new Date() },
    })
  }
  if (typeof hidden === 'boolean') {
    await ensureStageHiddenColumn()
    await prisma.$executeRawUnsafe(
      `UPDATE crm.crm_stage SET hidden = $1, "updatedAt" = now() WHERE id = $2 AND "tenantId" = $3`,
      hidden, stageId, tenantId,
    )
  }
...
/**
 * Apply a stage edit (name/code/color/thresholds and/or hidden) to the stage
 * with the SAME shortCode across EVERY pipeline in the tenant — the
 * "apply to all branches" option. Elevated roles only.
 */
export async function updateStageAllPipelines(
  tenantId: string,
  userId: string,
  shortCode: string,
  data: z.infer<typeof StageUpdateSchema>,
  ...
  let updated = 0
  if (Object.keys(prismaFields).length > 0) {
    const r = await prisma.crm_stage.updateMany({
      where: { tenantId, shortCode: code },
      data: { ...prismaFields, updatedAt: new Date() },
    })
    updated = r.count
  }
  if (typeof hidden === 'boolean') {
    ...
    const r = await prisma.$executeRawUnsafe(
      `UPDATE crm.crm_stage SET hidden = $1, "updatedAt" = now() WHERE "tenantId" = $2 AND "shortCode" = $3`,
      hidden, tenantId, code,
    )
    updated = Math.max(updated, typeof r === 'number' ? r : 0)
  }
  ...
  void logAudit({
    ...
    action: 'UPDATE',
    entity: 'crm_stage',
    meta: { scope: 'all-branches', shortCode: code, fields: Object.keys(parsed), updated },
  })
