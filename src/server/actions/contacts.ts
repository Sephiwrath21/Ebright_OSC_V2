...
export async function updateContact(
  ...
  try {
    ...
    // Snapshot the fields we log a change trail for (Email, Parent's Contact),
    // BEFORE the write, so we can record "from X to Y" as a visible note.
    const before = await prisma.crm_contact.findFirst({
    ...
    // Change-log notes for contact-detail edits — surfaced in the lead's Notes
    // so a branch can see it was changed, from what, to what, and by whom.
    if (before) {
      const trail: string[] = []
      if (input.email !== undefined) {
        const next = input.email === '' ? null : input.email
        if ((before.email ?? '') !== (next ?? '')) {
          trail.push(`Email changed from "${before.email ?? '—'}" to "${next ?? '—'}"`)
        }
      }
      if (normalizedPhone !== undefined && (before.phone ?? '') !== (normalizedPhone ?? '')) {
        trail.push(`Parent's Contact changed from "${before.phone ?? '—'}" to "${normalizedPhone ?? '—'}"`)
      }
      if (trail.length > 0) {
        // userId may be the literal 'api' for key-based edits — the note.user
        // relation is a real FK, so only attach a genuine user id.
        const noteUserId = userId && userId !== 'api' ? userId : null
        await prisma.crm_note.createMany({
          data: trail.map((body) => ({
            tenantId,
            contactId,
            userId: noteUserId,
            body: `✏️ ${body}${userEmail ? ` · by ${userEmail}` : ''}`,
          })),
        })
      }
    }
