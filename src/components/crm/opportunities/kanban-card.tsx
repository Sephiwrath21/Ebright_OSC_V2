...
import { GripVertical, User, Calendar, Check } from 'lucide-react'
import { useQaqc } from './qaqc-context'
...
export function KanbanCard({
  ...
  // QAQC verification tick — state + permissions come from the shared context
  // (KanbanBoard provides it; outside the board it defaults to hidden).
  const { canQAQC, get: getQaqc, toggle: toggleQaqc } = useQaqc()
  const qaqc = getQaqc(opportunity.id)
  const qaqcVerified = !!qaqc?.verified
  const showQaqc = canQAQC && showField('qaqc')
  ...
  return (
    ...
    >
      {/* QAQC verification tick — Super Admin / Regional Manager / Operation
          only. Grey when unverified, green when verified. Click toggles
          (verify; unverify is server-gated to super admins). */}
      {showQaqc && (
        <button
          type="button"
          onClick={(e) => { e.stopPropagation(); toggleQaqc(opportunity.id) }}
          title={
            qaqcVerified
              ? `QAQC verified${qaqc?.verifiedByName ? ` by ${qaqc.verifiedByName}` : ''}`
              : 'Mark QAQC verified'
          }
          aria-label="Toggle QAQC verified"
          className={cn(
            'absolute right-1.5 top-1.5 z-10 inline-flex h-5 w-5 items-center justify-center rounded-full ring-1 transition-colors',
            qaqcVerified
              ? 'bg-emerald-500 text-white ring-emerald-500 hover:bg-emerald-600'
              : 'bg-white text-slate-300 ring-slate-200 hover:text-emerald-500 hover:ring-emerald-300 dark:bg-slate-800 dark:text-slate-500 dark:ring-slate-600',
          )}
        >
      ...
      <div className={cn('flex items-start gap-1', compact ? 'p-1.5' : 'p-2.5')}>
        ...
        {/* Content */}
        <div className={cn('min-w-0 flex-1 space-y-1.5', showQaqc && 'pr-5')}>
          ...
          {prefs.quickActions.length > 0 && (
            <div className="flex items-center gap-2 pt-0.5">
              {prefs.quickActions.map((action) => {
                ...
                if (action === 'notes') badge = noteCount
                // Notes icon goes green once the lead has at least one note, so
                // a BM can see at a glance which cards have call/follow-up
                // context without opening each one.
                const hasNotes = action === 'notes' && noteCount > 0
                ...
                return (
                  <Link
                    ...
                    href={href}
                    onClick={(e) => e.stopPropagation()}
                    title={hasNotes ? `${noteCount} note${noteCount === 1 ? '' : 's'}` : action}
                    aria-label={ariaLabel}
                    className={cn(
                      'relative inline-flex h-6 w-6 items-center justify-center rounded transition-colors',
                      hasNotes
                        ? 'bg-emerald-100 text-emerald-600 hover:bg-emerald-200 dark:bg-emerald-900/50 dark:text-emerald-300'
                        : 'text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-700 dark:hover:text-indigo-300',
                    )}
                  >
                    <QuickActionIcon action={action} />
                    {badge !== null && badge > 0 && (
                      <span className={cn(
                        'absolute -right-1 -top-1 flex h-3.5 min-w-3.5 items-center justify-center rounded-full px-0.5 text-[8px] font-bold text-white',
                        hasNotes ? 'bg-emerald-600' : 'bg-indigo-600',
                      )}>
                        {badge > 9 ? '9+' : badge}
                      </span>
                    )}
