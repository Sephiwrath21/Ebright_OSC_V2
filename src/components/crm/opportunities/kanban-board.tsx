...
import { Plus, Search, X, Loader2, ChevronDown, Users, CalendarRange, CalendarDays, AlertTriangle, ArrowRight, MoveRight, PenLine, Settings2, ArrowDownWideNarrow, ArrowUpWideNarrow, Info, ShieldAlert, MoreHorizontal, Download, RotateCcw, MessageCircle } from 'lucide-react'
...
import { QaqcProvider, useQaqc, type QaqcContextValue } from './qaqc-context'
import { QaqcPanel } from './qaqc-panel'
import { getQaqcMap, setQaqcVerified, type QaqcState } from '@/server/actions/qaqc'
import type { KanbanStage, OpportunityCard } from '@/server/queries/opportunities'

const EMPTY_QAQC: QaqcState = { verified: false, verifiedAt: null, verifiedById: null, verifiedByName: null, result: null }
...
interface KanbanBoardProps {
  ...
  canDeleteLeads?: boolean
  /** QAQC tick visible + clickable (Super Admin / Regional Manager / Operation). */
  canQAQC?: boolean
  /** QAQC unverify + edit-date (Super Admin only). */
  canQAQCManage?: boolean
  /**
   * When set, cards may only be MOVED if their branch is in this list — every
   * other branch's board is view-only. Used by the Marketing account, which
   * sees all branches' kanbans but may only edit its own. null = edit anywhere
   * canEditLeads allows.
   */
...
// One stage column's worth of currently-shown (post-filter) opportunity IDs.
interface ExportStageGroup { id: string; name: string; shortCode: string; ids: string[] }

// 3-dot menu next to "New Opportunity": Export (detailed CSV) + Reset card layout.
function KanbanActionsMenu({ onResetCard, stageGroups }: { onResetCard: () => void; stageGroups: ExportStageGroup[] }) {
  const [open, setOpen] = useState(false)
  const [exportOpen, setExportOpen] = useState(false)
  const [exporting, setExporting] = useState(false)
  // Which stages to include in the export. Defaults to every stage that has at
  // least one shown card, so the plain "export everything" flow is one click
  // (open → Export) with no forced deselection.
  const [selected, setSelected] = useState<Set<string>>(new Set())

  // Only stages that actually have shown cards are worth offering.
  const exportable = stageGroups.filter((g) => g.ids.length > 0)
  const totalShown = exportable.reduce((n, g) => n + g.ids.length, 0)

  function openExport() {
    setSelected(new Set(exportable.map((g) => g.id)))   // all stages pre-checked
    setExportOpen(true)
    setOpen(false)
  }

  const selectedIds = exportable.filter((g) => selected.has(g.id)).flatMap((g) => g.ids)

  async function doExport() {
    if (selectedIds.length === 0) return
    setExporting(true)
    const res = await exportOpportunities(selectedIds)
    setExporting(false)
  ...
  const allChecked = exportable.length > 0 && selected.size === exportable.length
  ...
  return (
    <div className="relative">
      ...
      {/* Stage picker — choose which stages to export before downloading. */}
      {exportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/40" onClick={() => !exporting && setExportOpen(false)} />
          <div className="relative z-10 w-full max-w-sm rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
            <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 dark:border-slate-700">
              <div>
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Export to CSV</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Choose which stages to include.</p>
              </div>
              <button onClick={() => !exporting && setExportOpen(false)} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            {exportable.length === 0 ? (
              <p className="px-4 py-6 text-center text-sm text-slate-500 dark:text-slate-400">
                No leads on the board to export.
              </p>
            ) : (
              <>
                <div className="flex items-center justify-between border-b border-slate-100 px-4 py-2 dark:border-slate-800">
                  <label className="flex items-center gap-2 text-xs font-medium text-slate-600 dark:text-slate-300">
                    <input
                      type="checkbox"
                      checked={allChecked}
                      onChange={() => setSelected(allChecked ? new Set() : new Set(exportable.map((g) => g.id)))}
                  ...
                  <span className="text-[11px] text-slate-400">{selectedIds.length} of {totalShown} leads</span>
                </div>
                ...
                <div className="max-h-72 overflow-y-auto py-1">
                  {exportable.map((g) => (
                    <label
                      key={g.id}
                      className="flex cursor-pointer items-center gap-2.5 px-4 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-800"
                    >
                      <input
                        type="checkbox"
                        ...
                        className="h-3.5 w-3.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-700"
                      />
                      <span className="flex-1 truncate text-slate-700 dark:text-slate-200">{g.name}</span>
                ...
                <div className="flex items-center justify-end gap-2 border-t border-slate-200 px-4 py-3 dark:border-slate-700">
                  ...
                  >
                    ...
                    {exporting ? 'Exporting…' : `Export ${selectedIds.length}`}
                  </button>
                </div>
              </>
            )}
...
export function KanbanBoard({
  ...
  canQAQC = false,
  canQAQCManage = false,
  editableBranchIds = null,
}: KanbanBoardProps) {
  // A card is movable when there's no branch restriction, or its branch is in
  // the editable set. Marketing sees every branch but edits only its own.
  const canMoveBranch = useCallback(
    (branchId: string) => !editableBranchIds || editableBranchIds.includes(branchId),
    [editableBranchIds],
  )
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      // Branch-restricted movers (Marketing): may only move cards in their own
      // branch. The server enforces this too; this is the immediate feedback so
      // the card visibly snaps back instead of silently 403ing.
      const draggedCard = stages
        .flatMap((s) => s.opportunities)
        .find((o) => o.id === draggableId)
      if (draggedCard && !canMoveBranch(draggedCard.branchId)) {
        toast.error("You can only move your own branch's leads. Other branches are view-only.")
        return
      }
    ...
    [stages, moveMutation, pipelines, selectedPipelineId, canSwitchBranches, canEditLeads, canMoveBranch],
  )
  ...
  // ── QAQC verification state ────────────────────────────────────────────────
  // Verified state per visible opportunity, fetched in one batch keyed by id so
  // the existing kanban queries don't change. Provided via context so the
  // deeply-nested cards + the detail modal read it without prop threading.
  const [qaqcMap, setQaqcMap] = useState<Record<string, QaqcState>>({})
  const visibleOppIds = useMemo(
    () => filteredStages.flatMap((s) => s.opportunities.map((o) => o.id)),
    [filteredStages],
  )
  const visibleOppIdsKey = visibleOppIds.join(',')
  useEffect(() => {
    if (!canQAQC || visibleOppIds.length === 0) return
    let alive = true
    getQaqcMap(visibleOppIds).then((m) => { if (alive) setQaqcMap((prev) => ({ ...prev, ...m })) })
    return () => { alive = false }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [visibleOppIdsKey, canQAQC])

  const toggleQaqc = useCallback((oppId: string) => {
    setQaqcMap((prev) => {
      const cur = prev[oppId]
      const next = !cur?.verified
      // Optimistic — the server returns the authoritative verifier + timestamp.
      const optimistic: QaqcState = next
        ? { verified: true, verifiedAt: new Date().toISOString(), verifiedById: null, verifiedByName: 'You', result: cur?.result ?? null }
        : EMPTY_QAQC
      void setQaqcVerified(oppId, next).then((res) => {
        if (!res.ok) {
          setQaqcMap((p) => ({ ...p, [oppId]: cur ?? EMPTY_QAQC }))
          toast.error(res.error ?? 'QAQC update failed')
        } else if (res.state) {
          setQaqcMap((p) => ({ ...p, [oppId]: res.state! }))
        }
      })
      return { ...prev, [oppId]: optimistic }
    })
  }, [])

  const qaqcCtx = useMemo<QaqcContextValue>(() => ({
    canQAQC,
    canManage: canQAQCManage,
    get: (id) => qaqcMap[id],
    toggle: toggleQaqc,
    set: (id, state) => setQaqcMap((prev) => ({ ...prev, [id]: state })),
  }), [canQAQC, canQAQCManage, qaqcMap, toggleQaqc])

  // ── Render ───────────────────────────────────────────────────────────────────

  return (
    <QaqcProvider value={qaqcCtx}>
    <div className="flex h-full flex-col bg-white dark:bg-slate-900">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 dark:border-slate-700">
        ...
        <div className="flex items-center gap-2">
          ...
          <KanbanActionsMenu
            stageGroups={filteredStages.map((s) => ({
              id: s.id,
              name: s.name,
              shortCode: s.shortCode,
              ids: s.opportunities.map((o) => o.id),
            }))}
...
function OpportunityDetailModal({
  ...
  // QAQC panel — visible to Super Admin / Regional Manager / Operation. State +
  // caps come from the board's shared context so the card tick and this panel
  // stay in sync.
  const { canQAQC, canManage: canQaqcManage, get: getQaqc, set: setQaqc } = useQaqc()
  ...
  const [adminTab, setAdminTab] = useState<'details' | 'action' | 'whatsapp'>('details')
  ...
  // Inline Email + Parent's Contact editor — toggled by the pencil in the
  // Contact header. Saving writes a change-log note (server-side) that shows up
  // in this same drawer's Notes.
  ...
  async function handleSaveContact() {
    ...
    try {
      const res = await fetch(`/api/crm/contacts/${contact.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, phone: contactDraft.phone.trim() }),
      })
      if (!res.ok) {
        const body = await res.json().catch(() => ({}))
        throw new Error((body as { error?: string }).error ?? 'Failed to save')
  ...
  async function handleAddNote() {
    ...
    if (!contact?.id) { toast.error('This lead has no contact record to attach a note to.'); return }
    setSavingNote(true)
    try {
      ...
      if (!res.ok) {
        // Surface the server's actual reason (401/404/validation) instead of a
        // blanket "Failed to add note", which hid why the popup save failed
        // while the detail page — which already does this — worked.
        const errBody = await res.json().catch(() => ({}))
        throw new Error((errBody as { error?: string }).error ?? `Failed to save (HTTP ${res.status})`)
      }
      setNoteText('')
      // Refetch the opportunity so the new note appears immediately in the
      // popup, AND the whole board so the card's Notes icon turns green (its
      // note count rides on the kanban query, not the detail query).
      void queryClient.invalidateQueries({ queryKey: opportunityKeys.detail(opportunity.id) })
      void queryClient.invalidateQueries({ queryKey: opportunityKeys.all })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className={cn('relative z-10 flex w-full max-h-[85vh] overflow-hidden rounded-xl bg-white shadow-2xl border border-slate-200 dark:border-slate-700 dark:bg-slate-900', isSuperAdmin ? 'max-w-3xl' : 'max-w-lg')}>
        {/* Sidebar — Details | WhatsApp for everyone; Action is super-admin only. */}
        <nav className="flex w-32 shrink-0 flex-col gap-1 border-r border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/50">
          ...
          {([
            ...
            ...(isSuperAdmin ? [['action', 'Action', Settings2] as const] : []),
          ] as ReadonlyArray<readonly ['details' | 'whatsapp' | 'action', string, typeof Info]>).map(([key, label, Icon]) => (
            <button
              key={key}
        ...
        {/* Body — Details view (hidden when another tab is active) */}
        <div className={cn('space-y-4 px-5 py-4 text-sm', adminTab !== 'details' && 'hidden')}>
          {/* QAQC verification — Super Admin / Regional Manager / Operation. */}
          {canQAQC && (
            <QaqcPanel
              opportunityId={opportunity.id}
              initial={getQaqc(opportunity.id) ?? EMPTY_QAQC}
              caps={{ canVerify: canQAQC, canManage: canQaqcManage }}
              onChange={(s) => setQaqc(opportunity.id, s)}
            />
          )}
            ...
            <div className="grid grid-cols-[max-content_1fr] gap-x-3 gap-y-1.5">
              ...
              {isEditingContact ? (
                <input
                  ...
                  className="w-full rounded-md border border-slate-300 bg-white px-2 py-1 text-sm text-slate-900 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
                />
              ) : (
        ...
        {/* WhatsApp view — chat with this parent on the branch's own number */}
        {adminTab === 'whatsapp' && (
          <div className="px-5 py-4">
            <LeadWhatsAppPanel contactId={contact.id} />
          </div>
        )}
