...
export function TrialSchedule({ branchId, branches, readOnly = false }: TrialScheduleProps) {
  ...
  // Clicking a "Daily total" cell opens a per-branch breakdown for that day.
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          total === 0
                            ...
                            : 'cursor-pointer border-slate-300 bg-slate-100 text-slate-700 hover:border-indigo-400 hover:bg-indigo-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:hover:border-indigo-500 dark:hover:bg-indigo-950/30',
                        )}
                      >
      ...
      {/* Daily-total drill: trials-per-branch for the clicked day, descending. */}
      {branchTotals && (
        <BranchTotalsModal
          label={branchTotals.label}
          students={branchTotals.students}
          onClose={() => setBranchTotals(null)}
        />
      )}
    </section>
  )
...
//
// Opened from a "Daily total" cell. Aggregates that day's booked trials by
// branch and lists each branch with its count, highest first. No lead-level
// detail — just the branch tally, so a super-admin can see at a glance which
// branches are driving the day's trial volume.

function BranchTotalsModal({
  label,
  students,
  onClose,
}: {
  ...
  const rows = useMemo(() => {
    ...
    for (const s of students) {
      ...
      if (!g) {
        g = { branch, region: s.region, count: 0 }
        map.set(branch, g)
      }
      g.count++
    }
    return Array.from(map.values()).sort(
      (a, b) => b.count - a.count || a.branch.localeCompare(b.branch),
    )
  }, [students])
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-full max-w-md max-h-[85vh] overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 dark:ring-slate-700 dark:bg-slate-900">
        <header className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-4 dark:border-slate-800">
          <div className="min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
              Trial Class · Daily total
            </p>
            <h3 className="mt-0.5 text-lg font-bold text-slate-900 dark:text-white">{label}</h3>
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
              {students.length} trial{students.length === 1 ? '' : 's'} across {rows.length} branch
              {rows.length === 1 ? '' : 'es'}
            </p>
          </div>
          <button
        ...
        <ul className="divide-y divide-slate-100 overflow-y-auto px-2 py-1 dark:divide-slate-800" style={{ maxHeight: 'calc(85vh - 100px)' }}>
          {rows.length === 0 && (
            <li className="px-4 py-6 text-center text-sm text-slate-400">No trials booked.</li>
          )}
          {rows.map((r, i) => (
            <li key={r.branch} className="flex items-center gap-3 px-4 py-2.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-semibold text-slate-900 dark:text-white">
                  {r.branch}
                  {r.region ? <span className="font-normal text-slate-400"> · Region {r.region}</span> : null}
                </p>
              </div>
              <span className="inline-flex h-7 min-w-7 shrink-0 items-center justify-center rounded-lg bg-indigo-50 px-2 text-sm font-bold tabular-nums text-indigo-700 ring-1 ring-indigo-100 dark:bg-indigo-950/30 dark:text-indigo-300 dark:ring-indigo-900/40">
                {r.count}
              </span>
            </li>
          ))}
...
function SlotBreakdownModal({
  ...
  // Group by BRANCH only — one row per branch, not split by lead source. The
  // count badge is the branch's total for this slot; clicking drills into that
  // branch's students (where each student's source is still visible).
  const groups = useMemo(() => {
    const map = new Map<
      string,
      { branch: string; region: 'A' | 'B' | 'C' | null; students: Student[] }
    >()
    for (const s of students) {
      const branch = s.branchName ?? 'Unknown branch'
      let g = map.get(branch)
      if (!g) {
        g = { branch, region: s.region, students: [] }
        map.set(branch, g)
      }
      g.students.push(s)
    }
