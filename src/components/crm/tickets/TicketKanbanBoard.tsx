...
const DEPARTMENT_LABEL = new Map(DEPARTMENT_FILTER_OPTIONS.map((d) => [d.value, d.label]))

/**
 * Human-readable sub-type label for a card. Department sub_types get their
 * proper name ("human_resource" → "Human Resource"); other sub_types (issue
 * types like archive_student) are just de-underscored + title-cased.
 */
function formatSubType(subType: string): string {
  return (
    DEPARTMENT_LABEL.get(subType) ??
...
export function TicketKanbanBoard({ tickets: initialTickets, platforms }: TicketKanbanBoardProps) {
  ...
  // Branch/HQ scope filter. 'all' = every branch; 'hq' = internal departments
  // (reveals the Department sub-filter); otherwise a specific tkt_branch id.
  const [branchFilter, setBranchFilter] = useState<string>('all')
  const [hqDepartment, setHqDepartment] = useState<string>('all')
  const [search, setSearch] = useState('')
  const [moving, setMoving] = useState<string | null>(null)

  // Branches present in the current ticket set — the Branch dropdown's options,
  // sorted by the "NN" number. HQ (the internal departments) is prepended in
  // the render, ahead of these physical centres.
  const branchOptions = useMemo(() => {
    const map = new Map<string, { id: string; label: string; num: string }>()
    for (const t of tickets) {
      if (!map.has(t.branch.id)) {
        map.set(t.branch.id, { id: t.branch.id, label: `${t.branch.branchNumber} · ${t.branch.name}`, num: t.branch.branchNumber })
      }
    }
    return [...map.values()].sort((a, b) => a.num.localeCompare(b.num, undefined, { numeric: true }))
  }, [tickets])
  ...
  // Reset the HQ department sub-filter whenever we leave the HQ scope.
  useEffect(() => {
    if (branchFilter !== 'hq') setHqDepartment('all')
  }, [branchFilter])

  // Filter visible tickets by platform + branch/HQ scope + search.
  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase()
    return tickets.filter((t) => {
      if (platformId !== 'all' && t.platform.id !== platformId) return false
      // Branch/HQ scope. 'hq' shows only department-directed tickets (their
      // sub_type is a department slug), optionally narrowed to one department;
      // a specific branch id filters to that centre; 'all' applies no filter.
      if (branchFilter === 'hq') {
        if (!DEPARTMENT_LABEL.has(t.subType)) return false
        if (hqDepartment !== 'all' && t.subType !== hqDepartment) return false
      } else if (branchFilter !== 'all' && t.branch.id !== branchFilter) {
        return false
      }
      if (q) {
  ...
  }, [tickets, platformId, branchFilter, hqDepartment, search])
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900">
        ...
        {/* Branch dropdown — HQ (the internal departments) is the first option,
            then every centre present. Picking HQ reveals the Department filter. */}
        <div className="relative">
          <select
            value={branchFilter}
        ...
        {/* Department dropdown — appears only when HQ is the selected branch. */}
        {branchFilter === 'hq' && (
          <div className="relative">
            <select
              value={hqDepartment}
              onChange={(e) => setHqDepartment(e.target.value)}
              className="appearance-none rounded-lg border border-slate-300 bg-white pl-3 pr-8 py-1.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
            >
                            ...
                            {(provided, snapshot) => (
                              ...
                              >
                                ...
                                <div className="mt-2 flex items-center justify-between text-[10px] text-slate-400">
                                  <span className="truncate">{formatSubType(ticket.subType)}</span>
