...
import { DEPARTMENT_TKT_BRANCH_NUMBERS } from '@/lib/crm/departments'
...
export function TicketForm() {
  ...
  // Optional image attachment — offered on every platform (a screenshot helps
  // triage any ticket type).
  ...
  async function uploadTicketAttachment(ticketId: string, file: File) {
  ...
  const sessionUser = session?.user as
    | { tktRole?: string | null; departmentSubType?: string | null }
    | undefined
  const tktRole = sessionUser?.tktRole ?? 'user'
  const departmentSubType = sessionUser?.departmentSubType ?? null

  // Only a BRANCH account (files against a real centre) may raise platform
  // tickets (Aone / GHL / Lead / …). Department + HQ accounts, and anyone
  // scoped to a department, may only raise internal "Others → department"
  // tickets — so we show them just the Others platform on step 1. The server
  // (POST /api/crm/tickets) enforces the same rule; this is the UX half.
  //   • super_admin → unrestricted (all platforms).
  //   • scoped department account (departmentSubType set) → Others only.
  //   • otherwise → branch account iff a ticket branch is a real centre.
  //     While branches are still loading (empty) we assume branch to avoid a
  //     flash — a real department account always resolves to its dept branch,
  //     so it never misclassifies as branch here.
  // departmentSubType is checked FIRST so a department account that also holds
  // an elevated nav role (e.g. CEO gets super_admin nav breadth) is still
  // treated as non-branch here — matching the server, which blocks any
  // departmentSubType account from raising non-Others tickets.
  const isBranchAccount =
    !departmentSubType &&
    (tktRole === 'super_admin' ||
      branches.length === 0 ||
      branches.some((b) => !DEPARTMENT_TKT_BRANCH_NUMBERS.has(b.branch_number)))
  const visiblePlatforms = isBranchAccount
    ? platforms
    : platforms.filter((p) => isOthersPlatform(p.slug))
  ...
  // The topbar branch exists but has no matching ticket branch. crmBranchToTktBranchNumber
  // keys off the leading "NN " in the CRM branch name, so names without one —
  // "Ebright Marketing", "Ebright HR" — resolve to null and match nothing.
  //
  // The auto-fill below used to paper over that by picking branches[0], which
  // sorts to "00 Ebright (OD)". That is why tickets raised by the Marketing
  // account were filed as OD submissions. Refuse to guess instead.
  const branchUnmapped = branches.length > 0 && !!selectedBranch && !switcherLockedBranch
  ...
  useEffect(() => {
    ...
    // Never default an unmapped branch onto an arbitrary row — a mis-filed
    // ticket is worse than a blocked one.
    if (branchUnmapped) return
  ...
  }, [branches, branchId, canPickBranch, switcherLockedBranch, branchUnmapped, setValue])
  ...
  }, [showBranchField, branches, branchId, branchUnmapped, setValue])
  ...
  async function onSubmit(values: FormValues) {
    ...
    // Optional attachment — upload after the ticket exists. A failed upload
    // must NOT lose the ticket; warn and continue.
    if (attachment) {
      setUploadingAttachment(true)
      try {
        await uploadTicketAttachment(created.ticketId, attachment)
      } catch (e) {
        toast.error(
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {step === 'platform' && (
          <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800 sm:p-6">
          ...
          <div className="space-y-4 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800 sm:p-6">
            ...
            {showBranchField && (
              <div>
                ...
                {branchUnmapped ? (
                  <p className="mt-1 rounded-md border border-amber-300 bg-amber-50 px-2 py-1.5 text-[11px] text-amber-800 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-300">
                    <strong>{selectedBranch?.name}</strong> has no matching branch in the ticket
                    system, so this ticket can&apos;t be filed against it. Pick a different branch in
                    the topbar, or ask an admin to add the ticket branch.
                  </p>
                ) : !canPickBranch ? (
            ...
            {/* Optional image attachment — available on every platform. */}
            <div>
            ...
            <div className="flex justify-between pt-2">
              ...
              <Button type="submit" disabled={createMutation.isPending || uploadingAttachment || branchUnmapped}>
