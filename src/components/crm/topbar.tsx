...
import { DEPARTMENTS, isDepartmentBranch, departmentForBranchName } from '@/lib/crm/departments'
...
function BranchSwitcher({ user }: { user: SessionUser }) {
  ...
  function selectBranchAndNavigate(branch: BranchInfo | null) {
    ...
    setOpen(false)
    setQuery('')
    // The Opportunities kanban re-scopes itself reactively off BranchContext
    // (it switches to the picked branch's pipeline, or the All-Branches
    // aggregate when "View all branches" is chosen). Navigating away here would
    // wrongly bounce the user to the dashboard instead of updating the board —
    // exactly the "jumps to the dashboard, not the kanban" bug. Stay put and
    // let the board handle it.
    if (pathname === '/crm/opportunities') return
    // Everything else: push to the module-specific dashboard so the user isn't
    // stranded on a detail page that no longer applies to the new scope.
  ...
  // Sort numerically by the "NN …" prefix. Any branches without a numeric
  // prefix sort alphabetically below the numbered set.
  //
  // Department-owned branches (Ebright OD / Marketing / HR) are excluded from
  // "All accounts" for EVERYONE, not just operation accounts — they're internal
  // departments rather than centres, and the Departments row above is how you
  // reach them. Keeps the account list a list of real branches.
  // Marketing's switcher shows ONLY its own branch — it holds view links to
  // every branch (for the all-branch kanban) but the topbar is its home base,
  // not a branch picker. Everyone else gets the normal list minus the internal
  // department branches.
  const isMarketing = isMarketingAccount(user.email)
  const sorted = isMarketing
    ? branches.filter((b) => b.name === MARKETING_BRANCH_NAME)
    : [...branches]
        .filter((b) => !isDepartmentBranch(b.name))
        .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }))

  // Departments the viewer may actually open. A department with a CRM branch is
  // only offered if the viewer holds that branch (admins hold them all); the
  // ticket-only departments stay available to any admin.
  //
  // The Operation account only sees its OWN department chip — it has no reason
  // to jump into other departments' boards from its switcher.
  const visibleDepartments = DEPARTMENTS.filter(
    (d) =>
      (!d.branchName || branches.some((b) => b.name === d.branchName)) &&
      (!isOperation || d.subType === 'operation'),
  )

  /**
   * Departments are two different things depending on where you are:
   *  - in the ticket module a department is a ticket sub_type (no branch at
   *    all), so it opens that department's triage board — unchanged;
   *  - in the lead module a department with its OWN Nexus (Marketing,
   *    Optimisation) is a branch, so it scopes the CRM to that branch.
   * Departments without a Nexus land on /crm/no-nexus rather than an empty
   * board — an empty board reads as "my leads are missing".
   */
  function selectDepartment(d: (typeof DEPARTMENTS)[number]) {
    const inTicketModule = pathname.startsWith('/crm/tickets') || pathname.startsWith('/crm/tkt-')

    if (inTicketModule) {
      router.push(`/crm/tickets/kanban?dept=${d.subType}`)
      setOpen(false)
      setQuery('')
      return
    }

    const branch = d.branchName ? branches.find((b) => b.name === d.branchName) ?? null : null
    if (d.hasLeadSystem && branch) {
      selectBranchAndNavigate(branch)
      return
    }
    // Operation has the lead CRM but no branch of its own — it oversees ALL
    // branches. Clicking it lands on the all-branches lead dashboard, not the
    // no-Nexus page (which is only for tickets-only departments).
    if (d.hasLeadSystem && !branch) {
    ...
    router.push(`/crm/no-nexus?dept=${d.subType}`)
    setOpen(false)
    setQuery('')
  }
  ...
  // Non-admin with several branches (regional manager / multi-branch BM) and no
  // explicit selection is viewing ALL their branches — don't mislabel the panel
  // with the first branch's name ("Online"), which read as "locked to Online".
  const nonAdminDefaultLabel =
    branches.length > 1 ? 'All branches' : branches.length > 0 ? branches[0].name : 'My Branch'
  const defaultPanelLabel = !mounted
    ? isAdmin ? (isOperation ? 'Operation View' : 'Super Admin View') : (branches.length > 1 ? 'All branches' : 'My Branch')
    : isAdmin
      ? adminViewLabel
      : nonAdminDefaultLabel

  // Count only what the list actually shows — department branches are filtered
  // out of it, so counting `branches` would overstate by three.
  const adminBranchCount = sorted.length || '—'

  // A selected department branch reads as the department, not as a centre —
  // "Marketing" rather than "Ebright Marketing" sitting oddly among branches.
  const selectedDepartment = departmentForBranchName(selectedBranch?.name)

  const currentLabel = mounted
    ? (selectedDepartment?.name ?? selectedBranch?.name ?? defaultPanelLabel)
    : defaultPanelLabel
  ...
  // Ticket-module lock: a scoped department account (finance, hr, marketing,
  // operation, …) has no reason to switch branches or departments on ticket
  // pages — it triages only its own sub_type. Render a static "own department"
  // pill (no dropdown) instead of the full switcher. Its LEAD topbar is left
  // alone (this only fires inside /crm/tickets), so marketing/operation keep
  // their all-branches lead view. All hooks above have already run, so this
  // conditional return is safe.
  const inTicketModule = pathname.startsWith('/crm/tickets') || pathname.startsWith('/crm/tkt-')
  if (user.departmentSubType && inTicketModule) {
    return (
      <div className="relative">
        <div className="flex min-w-80 max-w-105 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2 text-left dark:border-slate-700 dark:bg-slate-800">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="truncate text-xs text-slate-500 dark:text-slate-400">Department tickets</div>
          </div>
        </div>
      </div>
    )
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-2 w-120 max-w-[90vw] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-800">
          ...
          {/* Agency-only: Manage access. Ticket access is a SEPARATE surface
              from CRM lead access — in the ticket module this routes to the
              ticket Users page (writes tkt_user_* tables), while the lead
              module keeps the CRM branch-access page (writes crm_user_branch).
              The two grant models never cross. */}
          {showViewToggle && effectiveViewMode === 'agency' && (
            <button
              onClick={() => { router.push(inTicketModule ? '/crm/tkt-users' : '/crm/settings/branch-access'); setOpen(false) }}
            ...
            >
              ...
              <span className="flex-1 text-left font-medium text-indigo-700 dark:text-indigo-300">
                {inTicketModule ? 'Manage ticket access' : 'Manage branch access'}
              </span>
          ...
          {/* "View all" option — admins see all their branches at once. Hidden
              for Marketing: its topbar is locked to its own branch (it still
              views every branch from the Opportunities pipeline selector). */}
          {branches.length > 1 && !isMarketing && (
            <button
          ...
          {/* Regions — admins only. Pins the Leads Dashboard to that region —
              the same view a regional manager of that region sees (headline +
              highlighted card scoped to the region, all three cards shown). The
              Day Distribution page is still reachable from the Region sidebar
              item; this dropdown jump is the "view as the RM" shortcut. */}
          {isAdmin && !isMarketing && (
              ...
              <div className="flex flex-wrap gap-1.5 px-3 pb-2">
                {(['A', 'B', 'C'] as const).map((r) => (
                  <button
                    key={r}
                    onClick={() => {
                      // Clear any specific-branch selection so the region scope
                      // isn't overridden by a lingering branch pick, then jump
                      // to the region-pinned dashboard.
                      setSelectedBranch(null)
                      router.push(`/crm/dashboard?region=${r}`)
                      setOpen(false)
                      setQuery('')
                    }}
          ...
          {/* Departments — admins only. In the lead module a department with a
              CRM branch scopes the CRM to it (that's how OD / Marketing / HR
              are reached now that they're out of the account list); in the
              ticket module, and for ticket-only departments, it opens that
              department's triage board. */}
          {isAdmin && !isMarketing && (
            <div className="border-t border-slate-100 dark:border-slate-700">
              <div className="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                Departments
              ...
              <div className="flex flex-wrap gap-1.5 px-3 pb-2">
                {visibleDepartments.map((d) => {
                  const isActive =
                    (!!d.branchName && selectedBranch?.name === d.branchName) ||
                    // The Operation account has no branch of its own — it IS the
                    // Operation view. Highlight its chip while it's on the
                    // default all-branches view so the dropdown reflects who's
                    // logged in, instead of only ticking "View all branches".
                    (d.subType === 'operation' && isOperation && !selectedBranch)
                  return (
                    <button
                      key={d.subType}
                      onClick={() => selectDepartment(d)}
                      title={d.branchName ? `${d.name} — ${d.branchName}` : `${d.name} tickets`}
                      className={cn(
                        ...
                        isActive
                          ...
                          : 'border-slate-200 bg-slate-50 text-slate-700 hover:border-indigo-300 hover:bg-indigo-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-indigo-500 dark:hover:bg-indigo-950/40',
                      )}
                    >
                      {d.name}
                      {isActive && <Check className="ml-1 inline h-3 w-3" />}
                    </button>
                  )
                })}
              </div>
          ...
          <div className="max-h-125 overflow-y-auto pb-1">
            {loading ? (
              ...
              filtered.map((branch: BranchInfo) => {
                ...
                // Per-branch share opens the CRM lead branch-access manager — a
                // lead-only concept. Ticket access is granted per-user on the
                // ticket Users page, so hide this in the ticket module.
                const showShare = showViewToggle && effectiveViewMode === 'agency' && !inTicketModule
                return (
                  <div
                    key={branch.id}
                    className={cn(
