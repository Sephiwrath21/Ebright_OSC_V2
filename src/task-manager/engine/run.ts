...
export async function completeBlock(input: CompleteBlockInput): Promise<CompleteBlockResult> {
  ...
  // a Daily task is only completable on its OWN due day — not once its
  // dueAt is strictly before today (completion closes for good), and not
  // before its dueAt has arrived either (server-authoritative; bits.tsx
  // mirrors both for the disabled UI, but this is the check that actually
  // can't be bypassed). Two separate checks, not one combined boolean, so
  // each direction gets its own accurate message instead of a generic one.
...
export async function skipBlock(input: SkipBlockInput): Promise<SkipBlockResult> {
  ...
  // Due-day lock (2026-08-05 past-day, extended 2026-08-11 to future-day —
  // the "Mark N/A" half of the same status control as completeBlock's own
  // guard above): server-authoritative, bits.tsx mirrors both for the
  // disabled UI.
  if (runBlock.cadence === "DAILY" && isPastDueDay(runBlock.dueAt)) {
...
export async function reopenBlock(input: ReopenBlockInput): Promise<ReopenBlockResult> {
  ...
  // Due-day lock (2026-08-05 past-day, extended 2026-08-11 to future-day):
  // "no status change action is available anymore, full stop" — a
  // past-day Daily task can't be reopened either, same as it can't be
  // completed or marked N/A in the first place. The future-day half is
  // effectively unreachable here in practice (a future-dated task can
  // never have REACHED DONE/SKIPPED in the first place, since
