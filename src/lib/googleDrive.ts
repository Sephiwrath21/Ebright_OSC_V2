...
// ─── Ticket media (its own Shared Drive folder, distinct from recruitment) ────

/** The ticket-media Shared Drive folder id, or null when unset. Separate from
 *  both GOOGLE_DRIVE_FOLDER_ID (proof photos) and the recruitment folder so
 *  ticket screenshots live in their own place. */
export function ticketDriveFolderId(): string | null {
  const id = process.env.GOOGLE_DRIVE_TICKET_FOLDER_ID?.trim();
  return id || null;
}
export function isTicketDriveConfigured(): boolean {
  return hasServiceAccount() && !!ticketDriveFolderId();
}

// ─── Finance / Petty Cash receipts (Shared Drive: EBRIGHT: 7.0 Finance) ───────

/** The Petty Cash "Receipt" folder id (holds the branch → month → day tree),
 *  or null when unset. Lives on the Finance shared drive. */
export function financeReceiptFolderId(): string | null {
  const id = process.env.GOOGLE_DRIVE_FINANCE_RECEIPT_FOLDER_ID?.trim();
  return id || null;
}
export function isFinanceDriveConfigured(): boolean {
  return hasServiceAccount() && !!financeReceiptFolderId();
}

/** List the immediate child FOLDERS of a Drive folder (shared-drive aware). */
...
/** Create a child folder (shared-drive aware) and return its id. */
export async function createChildFolder(parentId: string, name: string): Promise<string> {
  const drive = driveService();
  ...
  if (!id) throw new Error(`Google Drive folder create returned no id for "${name}"`);
  return id;
}
