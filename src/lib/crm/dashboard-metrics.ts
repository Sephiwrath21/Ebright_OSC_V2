...
// (showed up), SNE / CNS (showed-no-enrol / confirmed-no-show) and ENR
// (enrolled). RSD (Reschedule) is NOT counted — the old trial date is void once
// the lead is rescheduling, so it drops out of CT until a new date is
// confirmed (the move also deletes the old appointment). A lead that backed out
// before the trial (FU*, CL, DND, NL, UR_W*, …) is likewise not counted. The
// Trial Class Schedule grid is stricter: a lead occupies a live slot only in CT/SU.
export const CT_DASHBOARD_KEEP_CODES = new Set(['CT', 'SU', 'SNE', 'CNS', 'ENR'])
