...
export class TwilioWhatsAppProvider implements WhatsAppProvider {
  ...
  /** Fetches the account resource — proves the SID + auth token pair is valid. */
  async verifyCredentials(): Promise<VerifyResult> {
    const url = `https://api.twilio.com/2010-04-01/Accounts/${this.accountSid}.json`

    let response: Response
    try {
      response = await fetch(url, { headers: { Authorization: this.buildBasicAuth() } })
    } catch (err) {
    ...
    if (!response.ok) {
      return { ok: false, error: data.message ?? `HTTP ${response.status} from Twilio` }
    }

    return {
      ok: true,
      displayPhoneNumber: this.fromNumber,
      verifiedName: data.friendly_name,
    }
  }
  ...
  /**
   * Twilio exposes approved templates through the Content API, which is a
   * separate product with its own opt-in. Until that's wired up, report none —
   * the UI then keeps the branch on free-text sends inside the 24h window
   * rather than offering templates that would fail.
   */
