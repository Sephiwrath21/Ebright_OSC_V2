...
import type { PrismaClient } from '@/generated/crm-client'
...
export interface UnifiedLeadRow {
  ...
  remarks: string | null           // free-text remark the parent typed on the form, when the
                                   // source form has a remarks field; null otherwise
}
...
export interface ImportOptions {
  ...
  /**
   * Skip the bell + web-push "new lead" fan-out. A one-off backfill of
   * HISTORICAL rows (e.g. recovering roadshow siblings the old dedup dropped)
   * must not push hundreds of old leads to every admin's phone. The realtime
   * worker leaves this off so genuinely-new leads still notify.
   */
...
function pickContactName(row: UnifiedLeadRow): {
  ...
  // Default to 1 when sibling_index is NULL. The roadshow trial form submits ONE
  // child per submission with children_details = [{name, age}] but NO
  // sibling_index, and full_name holds the PARENT — so without this fallback
  // every sibling collapsed to the parent's name and the phone+name dedup merged
  // them into a single lead. Treating a null index as the first (only) child
  // makes each submission its own student.
  const idx = row.sibling_index ?? 1
  if (idx > 0) {
...
async function findDuplicateContactId(
  ...
  // When the incoming row carries an email, it becomes part of the identity:
  // a duplicate must match phone AND (child) name AND email. This is the
  // roadshow/wix rule — the same child with a different email (each submission
  // gets its own address) is a SEPARATE lead, not a merge. When there's no
  // email we fall back to phone + name so email-less sources (Meta/TikTok)
  ...
  const rows = await prisma.$queryRaw<{ id: string }[]>`
    SELECT id
     ...
     WHERE "tenantId" = ${tenantId}
       ...
       AND (${normEmail} = '' OR lower(btrim(coalesce(email, ''))) = ${normEmail})
     LIMIT 1
  `
...
export async function importLead(
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const contact = await tx.crm_contact.create({
        data: {
          ...
          // Free-text remark the parent typed on the form (new_website_form /
          // new_platform_leads / wix_trial_form_leads only — null for sources
          // with no remarks concept, e.g. Meta/TikTok).
          remarks:             row.remarks,
    ...
    // In-app bell + web-push notifications — one row per recipient (branch
    // users + elevated admins). Best-effort: the contact already exists, so
    // failing to write notifications shouldn't bubble up and roll anything back.
    // Skipped for historical backfills, which must not push old leads to phones.
    if (!opts.skipNotifications) try {
    ...
    if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
      try {
        ...
        if (row.remarks)       updates.remarks = row.remarks
