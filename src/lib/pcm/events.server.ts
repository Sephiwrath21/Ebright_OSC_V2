...
interface InvitationRow {
  ...
  attendance_proof_link: string | null;
...
function rowToInvitation(r: InvitationRow): Invitation {
  return {
    ...
    attendanceProofLink: r.attendance_proof_link ?? null,
...
// Self-provisioning columns — same pattern as paid/video_link/arrival_window
// before them (see prisma/sql/2026-05-23-pcm-invitation-paid.sql for the
// precedent). ADD COLUMN IF NOT EXISTS runs once per server process, before any
// read/write touches the columns, so no separate manual migration step is
// required.
//   package               — the renewal-package picker.
//   attendance_proof_link — Drive link to the coach's photo proof that the
//                           parent turned up (see prisma/sql/2026-07-15-pcm-attendance-proof.sql).
let invitationColumnsEnsured: Promise<void> | null = null;
function ensureInvitationColumns(): Promise<void> {
  if (!invitationColumnsEnsured) {
    invitationColumnsEnsured = pool.query(`
      ALTER TABLE pcm_invitations
        ADD COLUMN IF NOT EXISTS package               text,
        ADD COLUMN IF NOT EXISTS attendance_proof_link text
    `).then(() => undefined).catch((err) => {
      invitationColumnsEnsured = null;
      throw err;
    });
  }
  return invitationColumnsEnsured;
}

/**
 * Overlay each invitation's cached `coachName` with the coach's CURRENT full
 * name from HRFS branchstaff, resolved via the stored `coachId`.
 *
 * Why: `coach_name` is denormalised (cached at assignment time) and historically
 * held the coach's short nickname, so every PCM list — reports, dashboard,
 * inventory, invitations — rendered nicknames. Coaches live in a DIFFERENT
 * database (HRFS) than pcm_invitations, so we can't JOIN; we batch-look up the
 * ids here instead. This fixes existing rows too, without a destructive
 * backfill, and falls back to the cached name if HRFS is unreachable or the
 * coach id no longer exists.
 */
async function withCoachFullNames(invitations: Invitation[]): Promise<Invitation[]> {
  // v1 overlaid the coach's current full name from the HRFS branchStaff table
  // (a separate Prisma DB). v2's FA/PCM data layer is self-contained on the
  // leads DB and has no HRFS binding here, so we fall back to the coach_name
  // already snapshotted on each invitation row. This is display-only and does
  // not affect any count/tally.
  return invitations;
}
...
export async function fetchAllEventData(): Promise<{
  ...
  const [eventsRes, sessionsRes, quotasRes, invitationsRes, overridesRes] = await Promise.all([
    ...
    pool.query<InvitationRow>(
      `SELECT i.id, i.event_id, i.session_id, i.student_id, i.branch, i.target_grade,
              ...
              i.attendance_proof_link, i.arrival_window, i.arrival_time,
      ...
      return pool.query<InvitationRow>(
        `SELECT id, event_id, session_id, student_id, branch, target_grade, status, invited_by,
                ...
                attendance_proof_link, arrival_window, arrival_time,
  ...
  return {
    ...
    invitations: await withCoachFullNames(invitationsRes.rows.map(rowToInvitation)),
...
export async function updateInvitationRow(
  ...
  patch: {
    ...
    /** Drive link to the coach's photo proof that the parent attended
     *  (null clears it). Only meaningful while status is 'attended'. */
    attendanceProofLink?: string | null;
    /** Expected arrival window/time the parent comes to the branch (null clears). */
    arrivalWindow?: ArrivalWindow | null;
    arrivalTime?: string | null;
  }
): Promise<Invitation | null> {
  // status is included because a non-'attended' verdict clears the proof below,
  // and the reschedule branch clears it too — both touch attendance_proof_link.
  if (
    patch.package !== undefined ||
    patch.attendanceProofLink !== undefined ||
  ...
  if (patch.attendanceProofLink !== undefined) {
    fields.push(`attendance_proof_link = $${i++}`);
    values.push(patch.attendanceProofLink);
  }
  // The proof belongs to an 'attended' verdict. Moving off that verdict — Absent,
  // or Reset back to awaiting/confirmed — must not leave a "parent came" photo
  // attached to a student who is no longer marked present.
  if (patch.status !== undefined && patch.status !== "attended" && patch.attendanceProofLink === undefined) {
    fields.push(`attendance_proof_link = NULL`);
  }
  ...
  if (isReschedule) {
    ...
    // The old day's proof photo must not follow the student to the new date.
    fields.push(`attendance_proof_link = CASE WHEN status IN ('attended','no_show') THEN NULL ELSE attendance_proof_link END`);
  }
  ...
  const { rows } = await pool.query<InvitationRow>(
    `UPDATE pcm_invitations SET ${fields.join(", ")} WHERE id = $${i++} AND tenant_id = $${i}
     RETURNING id, event_id, session_id, student_id, branch, target_grade, status, invited_by,
               invited_at, confirmed_at, attendance_marked_at, attendance_marked_by, notes, invite_type, coach_id, coach_name, paid, package, video_sent_to_parent, video_link, attendance_proof_link, arrival_window, arrival_time`,
    values
  );
