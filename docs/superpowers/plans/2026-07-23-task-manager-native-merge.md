...
4. **BRANCH** login: branch roster + ad hoc section; `/task-manager/manpower-schedule` → create today's schedule, add a row + a Coach column, assign a coach to a cell, publish → cell shows the synced badge, and that coach's login now sees the shift task in their list.
